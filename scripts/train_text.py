...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train and evaluate a text emotion recognition model."
    )
    parser.add_argument(
        "--config",
        type=Path,
        required=True,
        help="Path to a text experiment config file.",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("outputs/runs/text"),
        help="Base directory for text run outputs.",
    )
    parser.add_argument(
        "--resume-from",
        type=Path,
...
def resolve_experiment_config(config_path: Path) -> dict[str, Any]:
    ...
    for entry in defaults:
        if not isinstance(entry, str):
            raise TypeError("For this phase, `defaults` entries must be string paths.")
...
def main() -> int:
    ...
    print(f"[OK] Finished text run: {run_dir}")
