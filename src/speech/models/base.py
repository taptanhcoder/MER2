...
class BaseSpeechClassifier(nn.Module):
    ...
    def resolve_num_classes(config: dict[str, Any]) -> int:
        ...
        raise KeyError(
            "Could not resolve num_classes for speech model. "
            "Expected one of: model.num_classes, dataset.num_classes, or label_space.labels."
        )
